...
import {
  HttpClientTestingModule, // mock module for HttpClientModule
...
describe("ProductService", () => {
  ...
  it("should return good response with data", (done) => {
    ...
    const productsRequest = httpMock.expectOne(
  ...
  it("should return good one product", (done) => {
    productService.getProduct(100).subscribe((product) => {
      expect(product.id).toBe(100);
      done();
    });

    const productsRequest = httpMock.expectOne(
      `${environment.apiEndPoint}/api/products/100`
    );
